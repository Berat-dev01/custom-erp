...
@section('title', __('Depo Düzenle'))
@section('page-title', __('Depo Düzenle'))

@section('content')
    <x-admin-panel::card>
        <form method="POST" action="{{ route('erp.warehouses.update', $warehouse) }}">
            @csrf @method('PUT')
            <div class="row g-3">
                <div class="col-md-8">
                    <x-admin-panel::input name="name" :label="__('Depo Adı')" :value="old('name', $warehouse->name)" required />
                </div>
                <div class="col-md-4">
                    <x-admin-panel::input name="code" :label="__('Kod')" :value="old('code', $warehouse->code)" required />
                </div>
                <div class="col-12">
                    <x-admin-panel::input name="address" :label="__('Adres')" :value="old('address', $warehouse->address)" />
                </div>
                <div class="col-md-6">
