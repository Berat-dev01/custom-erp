...
@section('title', __('Rol Düzenle').': '.$role->name)
@section('page-title', __('Rol Düzenle').': '.$role->name)
...
@section('content')
    ...
    <form method="POST" action="{{ route('erp.roles.update', $role) }}">
        @csrf @method('PUT')
        <x-admin-panel::card>
            <h6 class="fw-semibold mb-4">{{ __('Modül Bazlı Yetki Matrisi') }}</h6>
