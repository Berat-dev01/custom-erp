...
@section('title', __('Departman Düzenle'))
@section('page-title', __('Departman Düzenle'))

@section('content')
    <x-admin-panel::card>
        <form method="POST" action="{{ route('erp.departments.update', $department) }}">
            @csrf @method('PUT')
            <div class="row g-3">
                <div class="col-md-6">
                    <x-admin-panel::input name="name" :label="__('Departman Adı')" :value="old('name', $department->name)" required />
                </div>
                <div class="col-md-6">
                    <x-admin-panel::input name="code" :label="__('Kod')" :value="old('code', $department->code)" />
                </div>
                <div class="col-md-6">
